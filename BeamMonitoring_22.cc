...
extern dataChamber *chamber = new dataChamber;*/
 ...
 //TFile *CurrentHist = new TFile("CerrentHist.root", "RECREATE"); //
   ...
   for(int k = 0; k < counter; k++){ //The main loop: filling graphs by data point, filling the structure info by calculated values
    ...
    TGraphErrors* CalibrationCurve  = new TGraphErrors();
    TGraphErrors* CalibrationCurve1 = new TGraphErrors();
    ...
    int n = 0;
    int pc = 0;
    for(int j = 1; j <= 8; j++){ //The loop for filling graph by data from structute info

        pc = j - 1;
        CalibrationCurve->Set(pc);
        CalibrationCurve->SetPoint(pc, g[j].CorrespondCurrent, g[j].FluxProton);
        CalibrationCurve->SetPointError(pc, g[j].CorrespondCurrentError, g[j].ErFluxProton);

        scaleFactor->Set(pc);
        scaleFactor->SetPoint(pc, pc, g[j].ratio);
    ...
    TF1 *func = new TF1("f0","[0]*x",0, 1);
    func->SetParameter(0, 1.);

    CalibrationCurve->Fit(func, "R");
/*
    for(Int_t ip=0;ip<CalibrationCurve->GetN();ip++){
        cout<<"XXXXXXXX  "<<CalibrationCurve->GetX()[ip]<<"      "<<CalibrationCurve->GetY()[ip]<<endl;
    }
*/
...
/*
    for(Int_t ip=0;ip<CalibrationCurve1->GetN();ip++){
        cout<<"YYYYYYY  "<<CalibrationCurve1->GetX()[ip]<<"      "<<CalibrationCurve1->GetY()[ip]<<endl;
    }
*/
    ...
    Double_t calibrationScale  = func->GetParameter(0);
    Double_t calibrationScale1 = func05->GetParameter(0);
